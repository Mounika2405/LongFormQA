...
parser.add_argument('--task_type', help='type of task for LM: qda or qa')
...
def form_source_target(questions, documents, answers, output, dataset_name):
    ...
    qa_source = [questions[i] + " " + answers[i] for i, value in enumerate(documents)]
    qda_source = [questions[i] + " " + documents[i] + " " + answers[i] for i, value in enumerate(documents)]
    ...
    # write_output(output + "/" + dataset_name + ".qa_source", qa_source)
    # write_output(output + "/" + dataset_name + ".qa_target", answers)
    # write_output(output + "/" + dataset_name + ".qda_source", qda_source)
    # write_output(output + "/" + dataset_name + ".qda_target", answers)

def form_lm_source_target(questions, documents, answers, output, task_type,  dataset_name, valid=False):
    """
    ...
    if not valid:
        qd_source, targets = form_lm(questions, documents, answers, task_type)
        ...
        qd_source, targets = form_lm_valid(questions, documents, answers)

    # qd_source = lm_source + mask_source
    # targets = lm_target + mask_target
    ...
    write_output(output + "/" + dataset_name + ".lm_source", sources)
    write_output(output + "/" + dataset_name + ".lm_target", targets)
    return

def form_lm(questions, documents, answers, task_type):
    source = []
    target = []
...
def form_lm_valid(questions, documents, answers):
    """
...
if __name__ == "__main__":
    ...
    form_lm_source_target(train_q, train_d, train_a, args.output, args.task_type, "train")
    valid_q, valid_a, valid_d = read_data(args.input + "/explainlikeimfive_valid.json")
    form_lm_source_target(valid_q, valid_d, valid_a, args.output, args.task_type, "valid", True)
    test_q, test_a, test_d = read_data(args.input + "/explainlikeimfive_test.json")
    form_lm_source_target(test_q, test_d, test_a, args.output, args.task_type, "test", True)
